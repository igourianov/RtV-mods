...
func play(animation_state: String, audio_event: AudioEvent, wait_offset: float = -0.5) -> void:
	...
	var playback: AnimationNodeStateMachinePlayback = start_animation(animation_state)
	await await_animation(wait_offset, playback)
	Out.debug("play done")


func await_animation(wait_offset: float = 0.0, playback: AnimationNodeStateMachinePlayback = null) -> void:
	if !playback:
...
func start_animation(state_name: String):
	...
	var playback: AnimationNodeStateMachinePlayback = get_parent().animator.get("parameters/playback")
