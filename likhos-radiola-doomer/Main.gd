...
# Bootstrap: inject the Doomer station into Likho's Radiola, then get out of the
...
const STATION_DATA := "res://mods/likhos-radiola-doomer/doomer.tres"


func _ready() -> void:
	var station_script = load(STATION_SCRIPT)
	if !station_script:
		push_warning("[likho-radiola-doomer] Likho's Radiola not installed; station skipped")
		return
	var def = load(STATION_DATA)
	if !def:
		push_warning("[likho-radiola-doomer] failed to load station data")
		return
